...
# AlloyDB provisioning and database seeding are done manually.
# Usage: sh setup.sh
...
if [ -n "$ALLOYDB_REGION" ] && [ -n "$ALLOYDB_CLUSTER" ] && [ -n "$ALLOYDB_INSTANCE" ]; then
    ...
    if [ -n "$INSTANCES" ]; then
        ...
        echo ""
        echo "   Options:"
        echo "   1) Using a SHARED AlloyDB instance?"
        echo "      Enter the details provided by your instructor."
        echo "   2) Provision your own AlloyDB first (see README)."
        echo ""
        ...
        if [[ $REPLY =~ ^[1Yy]$ ]]; then
            ...
            echo "   Please provision AlloyDB first (see README), then re-run this script."
            exit 1
        fi
    fi
fi
...
echo ""
echo "Next steps:"
echo "  1. Set up your database schema in AlloyDB Studio (see database/seed_data.sql)"
echo "  2. Configure your .env file with the required environment variables"
